...
def test_a_service_that_ignores_the_tags_is_rejected():
    """The failure mode that matters: the terms come back translated anyway."""
    ...
    with pytest.raises(glossary.GlossaryViolation):
        translate_prose("This unit has a hurdle requirement.", Careless())


def test_the_echo_translator_passes_the_check_and_is_still_not_a_translation():
    """Echo plus the glossary yields English prose with Chinese terms in it.

    It passes every check, which is exactly why it is confined to dry runs and
    why `run()` refuses to commit with it - see the guard in translate_units.py.
    """
    result = translate_prose("This unit has a hurdle requirement.", EchoTranslator())
    assert "必过项" in result
    assert "has a" in result  # still English, and that is the point
